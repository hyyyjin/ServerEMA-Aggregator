...
public class Mqtt_ClientEMA extends Thread implements MqttCallback {
	...
	public Mqtt_ClientEMA(String brokerUrl, String clientId, boolean cleanSession, boolean quietMode, String userName,
	...
	public void messageArrived(String topic, MqttMessage message)
		...
		if (msgPayload.startsWith("{") && msgPayload.endsWith("}")) {
			try {
				...
				if (topicParse[1].equals("EMAP") && topicParse[2].equals(global.CHILD_ID)) {
					...
					if (msg_json.getString("DestEMA").equals(global.CHILD_ID)) {
						...
						case Report:
							...
							break;
						case Opt:
							String optID = msg_json.getString("optID");
							opt(procedure, profileVersion, optID);
							break;
						case Event:
							poll(procedure, profileVersion);
							break;
				...
				else if (topicParse[1].equals("OpenADR") && topicParse[2].equals(global.CHILD_ID)) {

					String profileVersion = "OpenADR2.0b_new";
					...
					if (procedure.matches(
							"CreatedPartyRegistration|RegisteredReport|RegisterReport|Response|DistributeEvent")) {
						...
						if (procedure.equals("Response")) {
							...
							if (global.venRegisterSeqNumber == 0) {
				...
				if (global.reportType.equals("Explicit")) {
					...
					while (it.hasNext()) {
						...
						rd.addReportDescriptionParams(key, key, "CEMA", global.reportType, "itemUnits", "siScaleCode",
								...
								pa.getPowerAttributesParams(), "state", global.emaProtocolCoAP.get(key).getqOs(),
								global.emaProtocolCoAP.get(key).getPower(),
								global.emaProtocolCoAP.get(key).getDimming(),
								global.emaProtocolCoAP.get(key).getMargin(),
								global.emaProtocolCoAP.get(key).getGenerate(),
								global.emaProtocolCoAP.get(key).getStorage(),
								global.emaProtocolCoAP.get(key).getMaxValue(),
								global.emaProtocolCoAP.get(key).getMinValue(),
	...
	public void opt(String procedure, String profileVersion, String optID) {
...
//			Random rand = new Random();
			String topic = "/EMAP/" + "DEVICE1" + "/1.0b/Opt";

			String payload = createdOpt.toString();
//			System.out.println("보냇니");

			new Publishing().publishThread(this.connection.getEmapmqttclient().getMqttClient(), topic, global.qos,
					payload.getBytes());
	...
	public void summaryReport(String procedure, String profileVersion) {
		...
		if (profileVersion.equals("EMAP1.0b")) {
			...
			sr.setSrcEMA(global.CHILD_ID);

			String topic = "/EMAP/" + global.ParentnNodeID + "/1.0b/Summary";
	...
	public void sendRDR(String optID) {

		if (!global.rdrMap.keySet().contains(optID)) {
			global.rdrMap.put(optID, 1);
		} else {
			int cnt = global.rdrMap.get(optID).intValue();
			global.rdrMap.replace(optID, cnt + 1);
		}

		com.mir.ems.profile.emap.v2.Available available = new com.mir.ems.profile.emap.v2.Available();
		available.addEventParams(getCurrentTime(System.currentTimeMillis()), "PT10S", 90.0, 20181127, 165520, 20181127,
				165520);
		...
		String topic = "/EMAP/" + global.ParentnNodeID + "/1.0b/Opt";
